...
async def show_templates_handler(query: CallbackQuery, bot, **kwargs):
    ...
    if not templates:
        keyboard.add(InlineKeyboardButton("Назад", callback_data=menu_cd.new(action="templates")))
    ...
    for template in templates:
        if(template["is_select"]):
            button = InlineKeyboardButton(template["template_name"] + " ✅", callback_data=template_cd.new(template_id=template["id"], action="edit_template_button"))
        else:
            button = InlineKeyboardButton(template["template_name"], callback_data=template_cd.new(template_id=template["id"], action="edit_template_button"))
        keyboard.add(button)
...
async def delete_templates_handler(query: CallbackQuery, bot, **kwargs):
    ...
    for template in templates:
        if(template["is_select"]):
            button = InlineKeyboardButton(template["template_name"] + " ✅", callback_data=template_cd.new(template_id=template["id"], action="delete_template_button"))
        else:
            button = InlineKeyboardButton(template["template_name"], callback_data=template_cd.new(template_id=template["id"], action="delete_template_button"))
        keyboard.add(button)
...
async def edit_template_handler(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    keyboard = InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        InlineKeyboardButton("Ключевые слова шаблона", callback_data=template_cd.new(template_id=template_id, action="list_keywords_from_tamplate")),
        InlineKeyboardButton("Слова-исключения шаблона", callback_data=template_cd.new(template_id=template_id, action="list_stopkeywords_from_tamplate")),
        InlineKeyboardButton("Добавить ключевые слова", callback_data=template_cd.new(template_id=template_id, action="add_keywords_to_tamplate")),
        InlineKeyboardButton("Добавить слова-исключения", callback_data=template_cd.new(template_id=template_id, action="add_stopwords_to_tamplate")),
        InlineKeyboardButton("Удалить ключевые слова", callback_data=template_cd.new(template_id=template_id, action="remove_keywords_from_tamplate")),
        InlineKeyboardButton("Удалить слова-исключения", callback_data=template_cd.new(template_id=template_id, action="remove_stopwords_from_tamplate")),
        InlineKeyboardButton("Группы шаблона", callback_data=template_cd.new(template_id=template_id, action="groups_from_tamplate")),
        InlineKeyboardButton("Выбрать основным", callback_data=template_cd.new(template_id=template_id, action="select_template")),
        InlineKeyboardButton("Назад", callback_data=menu_cd.new(action="show_templates_button"))
    )
...
async def select_template_handler(query: CallbackQuery, callback_data: dict, bot, **kwargs):
    ...
    if(selected_template):
        if (int(template_id) == selected_template[0]):
            back_markup = InlineKeyboardMarkup().add(InlineKeyboardButton("Назад", callback_data = template_cd.new(template_id = template_id, action = "edit_template_button")))
...
async def groups_from_tamplate(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        InlineKeyboardButton("Список групп шаблона", callback_data=template_cd.new(template_id=template_id,  action="list_groups_template")),
        InlineKeyboardButton("Добавить по ссылке", callback_data=template_cd.new(template_id=template_id, action="add_groups_link")),
        InlineKeyboardButton("Добавить поиском", callback_data=template_cd.new(template_id=template_id, action="add_groups_search")),
        InlineKeyboardButton("Удалить группу", callback_data=template_cd.new(template_id=template_id, action="remove_group")),
        InlineKeyboardButton("Назад", callback_data=template_cd.new(template_id=template_id, action="edit_template_button"))
    )
...
async def list_groups_template(query: CallbackQuery, callback_data: dict, **kwargs):
    ...
    if not groups:
        ...
        group_list = "\n".join([f"id: {group[0]} Название: {group[1]}" for group in groups])
        text = f"Группы этого шаблона:\n{group_list}"

    back_markup = InlineKeyboardMarkup().add(InlineKeyboardButton("Назад", callback_data=template_cd.new( template_id=template_id, action="groups_from_tamplate")))
...
async def remove_group_template(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    await query.answer()
    user_data[query.from_user.id] = "remove_group"
    user_data["edit_template"] = callback_data.get('template_id')
...
async def group_ids_to_remove_template(message: types.Message, user_data, **kwargs):
    ...
    if groups_to_remove:
        for group_id in groups_to_remove:
            await remove_group_template(message.from_user.id, group_id, template_id)
...
async def join_group_by_search_handler_template_msg(msg: types.Message, user_data, current_client, **kwargs):
    ...
    if not group_name:
        ...
        if not groups:
            ...
            groups_data[add_by_userid] = {'groups': groups, 'template_id': template_id}
            group = groups[0]
            group_id = group.id
            ...
            buttons.append(InlineKeyboardButton("Назад", callback_data=template_cd.new( template_id=template_id, action="groups_from_tamplate")))
...
async def process_add_group_button(call: aiogram.types.CallbackQuery, current_client, **kwargs):
    ...
    if user_groups is not None:
        group = next((group for group in user_groups if group.id == group_id), None)


    isJoined = await join_group_by_id(current_client, group_id)
    if (isJoined):
        try:
            await add_group_template(group.id, group.title, add_by_userid, template_id)
...
async def add_keywords_to_tamplate_handler(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    user_data[query.from_user.id] = {"edit_template": template_id, "template_name": template_name, "add_keywords_template": True}
...
async def add_blacklistkeyword_to_tamplate_handler(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    user_data[query.from_user.id] = {"edit_template": template_id, "template_name": template_name, "add_blacklistkeyword_template": True}
...
async def remove_keywords_from_tamplate_handler(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    user_data[query.from_user.id] = {"edit_template": template_id, "template_name": template_name, "remove_keywords_template": True}
...
async def remove_keywords_from_tamplate(msg: types.Message, user_data, bot, **kwargs):
    ...
    for keyword in keywords:
        keyword = keyword.strip()
        if await remove_keyword_from_template(user_id, keyword, template_id):
...
async def remove_blacklistkeywords_from_template_handler(query: CallbackQuery, callback_data: dict, user_data, bot, **kwargs):
    ...
    user_data[query.from_user.id] = {"edit_template": template_id, "template_name": template_name, "remove_blacklistkeyword_template": True}
...
async def remove_blacklistkeywords_from_template(msg: types.Message, user_data, bot, **kwargs):
    ...
    for keyword in keywords:
        keyword = keyword.strip()
        if await remove_blacklistkeyword_from_template(user_id, keyword, template_id):
            added_keywords.append(keyword)
        else:
            failed_keywords.append(keyword)
